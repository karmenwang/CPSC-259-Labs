...
void generate_all_paths( maze_cell ** maze, int dimension, int row, int column, char * path )
{
    ...
    if ( maze[row][column].visited==VISITED||maze[row][column].character == '*' ){
        return;
	}
...
int path_cost ( char * path_string )
{
	...
	if (*path_string == NULL)
		...
		return *path_string + path_cost(path_string + 1) - 48;	//offset value
...
void display_shortest_path (){
	int i = 0;
	int size;
	int length = strlen(paths[0]);
	char *shortest_path = (char*)malloc(1000*sizeof(char));
	shortest_path = *paths;
...
{
	int i;
	int cheapest_path = paths[0];
    int cheapest_cost = path_cost(paths[0]);
	int cost;
	for (i = 0; i < paths_found; i++) {
