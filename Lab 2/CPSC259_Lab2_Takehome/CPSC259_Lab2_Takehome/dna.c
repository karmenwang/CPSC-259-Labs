...
int calculate_score(char * sample_segment, char * candidate_segment)
{
	...
	int score = 0;
	int iterations = 0;
	int sample_length = strlen(sample_segment) - (strlen(sample_segment)%3);
	...
	int index_sample;
	int index_candidate;
	char* sample_codon = malloc(3 * sizeof(char));
	char* sample_candidate = malloc(3 * sizeof(char));


	/* This is the control for the outer for loop*/
	for (iterations = 0; iterations <= (candidate_length_in_codons - sample_length_in_codons + 1)*3; iterations+=3){

		//checks whether tempscore is greater than score, if so score gets value of temp score and temp score is reset to 0
		score = max(temp_score, score);
		temp_score = 0;

		//loop that controls shifting candidate's position by a codon
		for (i = 0; i < sample_length; i +=3) {

			//every 3 index of sample_segment is copied to sample codon
			sample_codon[0] = sample_segment[i];
			sample_codon[1] = sample_segment[i + 1];
			sample_codon[2] = sample_segment[i + 2];

			//every 3 index of sample_candidate is copied to candidate_segment
			sample_candidate[0] = candidate_segment[iterations + i];
			sample_candidate[1] = candidate_segment[iterations + 1 + i];
			sample_candidate[2] = candidate_segment[iterations + 2 + i];

			// Index of the codon, which will correspond to an index in the codon_names array
			index_sample = get_codon_index(sample_codon);
			index_candidate = get_codon_index(sample_candidate);

			// Compares each codon from sample with the respective codon in the candidate
			if (strncmp((sample_segment + i), (candidate_segment + (iterations + i)), 3) == 0) {
				temp_score += 10;
			}

			// checks to see if the two codons have the same name
				...
				// loop goes through the individual codons and compares with nucleotides

				for (j = 0; j < CODON_LENGTH; j++) {

					//checks for nucleotide matches
					if (sample_codon[j] == sample_candidate[j]) {
						temp_score += 2;
					}

					//checks if specified nucleotides are a base pair by calling function is_base_pair
					else if (is_base_pair(sample_codon[j], sample_candidate[j]) == 1) {
						temp_score += 1;
					}
				}
			}
